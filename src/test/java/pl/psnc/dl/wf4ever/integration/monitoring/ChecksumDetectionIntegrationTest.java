...
public class ChecksumDetectionIntegrationTest extends AbstractIntegrationTest {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ChecksumDetectionIntegrationTest.class);
	...
	public final void test() throws FeedException, IOException,
		...
		int seconds = 0;
		DateTime from = entries.isEmpty() ? null : new DateTime(entries.get(
				entries.size() - 1).getPublishedDate()).plusSeconds(1);
		while (++seconds < 10) {
		...
		SyndEntry entry = (SyndEntry) feed.getEntries().get(
				feed.getEntries().size() - 1);
		Assert.assertTrue(
				entry.getTitle(),
				entry.getTitle().matches(
						"Research Object .+ has become corrupt!"));
		Assert.assertTrue(
	...
	private String modifyFile(String filePath2) throws IOException {
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(
			...
			DigitalLibrary dl = Mockito.spy(new FilesystemDLFactory(properties)
					.getDigitalLibrary());
			// don't update the db
			Mockito.doReturn(null).when(dl)
					.updateFileInfo(ro, filePath2, "text/plain");
			String text = "lorem ipsum modified";
			dl.createOrUpdateFile(ro, filePath2, IOUtils.toInputStream(text),
					"text/plain");
