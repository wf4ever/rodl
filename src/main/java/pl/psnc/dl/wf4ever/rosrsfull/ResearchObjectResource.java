...
public class ResearchObjectResource {
     ...
     * @return 200 (OK) response code with a rdf file in response body containing OAI-ORE aggreagates tags.
    ...
    /**
     * Creates new version. Input is RO_VERSION_ID and optional URI of the base version that should be used to create a
     * new version.
     * 
     * @param workspaceId
     * @param researchObjectId
     * @param data
     *            Input format is text/plain with RO_VERSION_ID in first line and base version URI in second (optional).
     * @return 201 (Created) if the version was created, 409 (Conflict) if version with given RO_VERSION_ID already
     *         exists
     * @throws IOException
     * @throws TransformerException
    ...
    public Response createVersion(@PathParam("W_ID") String workspaceId, @PathParam("RO_ID") String researchObjectId,
        ...
        try {
            if (baseVersion == null) {
                ...
                InputStream manifest = sms.getManifest(roURI.resolve(".ro/manifest.rdf"), RDFFormat.RDFXML);
