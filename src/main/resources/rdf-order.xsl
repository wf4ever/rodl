...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template match="rdf:Description">
  <xsl:copy>
   <xsl:apply-templates select="@*"/>
   <xsl:apply-templates select="*[name() != 'ore:aggregates' and name() != 'dcterms:hasVersion' and name() != 'oxds:currentVersion' and name() != 'dcterms:modified' and name() != 'rdf:type']">
    <xsl:sort select="name()"/>
   </xsl:apply-templates>
   <xsl:comment> RO SRS generated tags below </xsl:comment>
